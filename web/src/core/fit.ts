...
type BaseTypeRef = string | BaseType;
...
function dateToFitTimestamp(date: Date | null | undefined): number {
  if (!date) return 0;
  return Math.floor(date.getTime() / 1000) - FIT_EPOCH_S;
}

function fitTimestampToDate(ts: number): Date {
  return new Date((ts + FIT_EPOCH_S) * 1000);
}

function isFreeRideSegment(seg: unknown): boolean {
...
function pushUint(bytes: number[], value: number, size: number): void {
...
function encodeString(str: string, size?: number): number[] {
  ...
  for (let i = 0; i < len; i++) out.push(buf[i] as number);
  if (size) {
    out.push(0);
...
function normalizeBaseType(type: BaseTypeRef | undefined): BaseType {
  if (typeof type === 'string') {
    return BASE_TYPES[type] as BaseType;
  }
  return type as BaseType;
}

function baseTypeFromId(id: number | null | undefined): BaseType {
  if (id == null) return BASE_TYPES.byte as BaseType;
  const baseNum = id & 0x1f;
  return (
    Object.values(BASE_TYPES).find((t) => (t.id & 0x1f) === baseNum) ||
    (BASE_TYPES.byte as BaseType)
  );
}
...
function encodeValue(
  ...
  switch (baseType.id) {
    ...
    case BASE_TYPES.sint16!.id:
      pushUint(bytes, val as number, 2);
      break;
    case BASE_TYPES.uint32!.id:
    case BASE_TYPES.uint32z!.id:
    case BASE_TYPES.sint32!.id:
    case BASE_TYPES.float32!.id:
      pushUint(bytes, val as number, 4);
      break;
    case BASE_TYPES.string!.id: {
      const encoded = encodeString((val as string) || '', size);
      bytes.push(...encoded);
      break;
    }
    default:
      pushUint(bytes, val as number, size || baseType.size);
      break;
  }
  while (bytes.length < (size || baseType.size)) bytes.push(0x00);
  return bytes.slice(0, size || baseType.size);
...
function createDataMessage(
  def: Definition,
  fieldValues: Record<string | number, unknown> = {},
  devFieldValues: Record<string | number, unknown> = {},
): number[] {
  const bytes: number[] = [];
  ...
  if (def.hasDevFields) {
    def.devFields.forEach((f) => {
      ...
      const v =
        ...
        devFieldValues[key] ??
        (f.name != null ? devFieldValues[f.name] : undefined);
      bytes.push(...encodeValue(base, size, v));
    });
  }
  return bytes;
...
function prepareDeveloperFieldDescriptions(): {
  ...
  const fields: DevFieldDesc[] = [
    {
      num: 0,
      name: 'vd_tgt_pow',
      type: BASE_TYPES.uint16 as BaseType,
      units: 'watts',
    ...
    },
    {
      num: 3,
      name: 'vd_source',
      type: BASE_TYPES.string as BaseType,
      units: '',
      size: 64,
...
export interface BuildFitOptions {
  canonicalWorkout?: Partial<CanonicalWorkout> | null;
  samples?: FitSample[];
  ftp?: number | null;
  startedAt?: Date | number | string | null;
  endedAt?: Date | number | string | null;
  pauseEvents?: { at?: Date | number | string; type?: string }[];
...
export function buildFitFile({
  ...
  pauseEvents = [],
  totalElapsedSec,
}: BuildFitOptions): Uint8Array {
  const bytes: number[] = [];

  const cw = (canonicalWorkout || {}) as Partial<CanonicalWorkout>;
  const startDate =
    startedAt instanceof Date
      ...
      : startedAt
        ...
        : samples.length
          ? new Date(
              Date.now() -
                ((samples[samples.length - 1] as FitSample).t || 0) * 1000,
            )
          : new Date();
  const computedElapsedMs =
    totalElapsedSec != null
      ...
      : samples.length
        ? Math.max(0, (samples[samples.length - 1] as FitSample).t || 0) * 1000
        : 0;
  const endDate =
  ...
  const totalWorkJ = samples.reduce((sum, s, idx) => {
    ...
    const prevT = idx > 0 ? (samples[idx - 1] as FitSample).t || 0 : 0;
    const dt = Math.max(1, Math.round((s.t || 0) - prevT));
    return sum + (s.power as number) * dt;
  }, 0);
  ...
  canonicalChunks.forEach((chunk, idx) => {
    devInfo.fields.push({
      ...
      name: `vd_canon${idx}`,
      type: BASE_TYPES.byte as BaseType,
      units: '',
      nativeMesgNum: 26,
      size: chunk.length,
    });
  ...
  const workoutDevFields: FieldDef[] = [];
  devInfo.fields
    ...
    .forEach((f) => {
      workoutDevFields.push({
        ...
        type: f.type || (BASE_TYPES.byte as BaseType),
  ...
  const workoutStepDevFields: FieldDef[] = [
  ...
  const recordDevFields: FieldDef[] = [
  ...
  // workout
  const workoutDevValues: Record<number, unknown> = {};
  devInfo.fields.forEach((f) => {
  ...
  (cw.rawSegments || []).forEach((seg, idx) => {
    const arr = seg as unknown as unknown[];
    const minutes = arr[0] as number;
    const startPct = arr[1] as number;
    const endPct = arr[2] as number;
    const stepDurationSec = Math.max(1, Math.round((minutes || 0) * 60));
    const isFreeRide = isFreeRideSegment(seg);
    const ftpVal = ftp || 0;
    const startPctVal = isFreeRide ? 50 : startPct || 0;
    const endPctVal = isFreeRide
      ? 50
      : endPct != null
        ? endPct
        : startPct || 0;
  ...
  const timerEvents: { type: string; ts: number }[] = [];
  ...
  normalizedPauses
    ...
    .filter((x): x is { type: string; ts: number } => Boolean(x))
...
function parseHeader(dataView: DataView, offset = 0): ParsedHeader {
...
interface ParsedDefinition {
  localMsgNum: number;
  globalMsgNum: number;
  fields: { num: number; size: number; type: BaseType }[];
  devFields: { num: number; size: number; devIndex: number }[];
  hasDevFields: boolean;
  little: boolean;
}
...
function parseDefinition(
  ...
  localMsgNum: number,
  limit: number,
): { nextOffset: number; def: ParsedDefinition | null } {
  if (offset + 5 > limit) {
  ...
  const fields: { num: number; size: number; type: BaseType }[] = [];
  let cursor = offset + 5;
  ...
  const devFields: { num: number; size: number; devIndex: number }[] = [];
  if (hasDevFields) {
    if (cursor + 1 > limit) {
      return { nextOffset: limit, def: null };
    }
...
export interface ParseFitResult {
  ...
  meta: {
    ...
    startedAt: Date | null;
    endedAt: Date | null;
    totalWorkJ: number | null;
    totalElapsedSec: number | null;
    totalTimerSec: number | null;
    pauseEvents: { type: string; at: Date }[];
  };
}

export function parseFitFile(arrayBuffer: ArrayBuffer): ParseFitResult {
  ...
  const defs = new Map<number, ParsedDefinition>();
  const devFieldInfo = new Map<
    string,
    {
      name: string;
      fitBaseTypeId: number;
      nativeMesgNum: unknown;
      nativeFieldNum: unknown;
    }
  >();
  const recordSamples: {
    t: number;
  ...
  const workoutMeta: { name?: string } = {};
  const workoutDevFieldValues: Record<string, unknown> = {};
  const sessionValues: Record<number, unknown> = {};
  const timerEvents: { ts: number; type: number }[] = [];
  ...
  while (offset < limit) {
    ...
    let cursor = offset;
    def.fields.forEach((f) => {
      const base = f.type || (BASE_TYPES.byte as BaseType);
    ...
    const devValues: Record<string, unknown> = {};
    if (def.hasDevFields) {
      def.devFields.forEach((f) => {
    ...
    if (isCompressed && timeOffset != null && lastTimestamp != null) {
      const prevLsb: number = lastTimestamp & 0x1f;
      const base: number = lastTimestamp & 0xffffffe0;
      const ts: number =
        timeOffset >= prevLsb ? base + timeOffset : base + timeOffset + 0x20;
      if (values[253] == null) values[253] = ts;
      lastTimestamp = ts;
    } else if (values[253] != null) {
      lastTimestamp = values[253] as number;
    }

    switch (def.globalMsgNum) {
      case 206: {
        const name = (values[3] as string) || '';
        const devIndex = (values[0] as number) || 0;
        const fieldNum = (values[1] as number) || 0;
        const fitBaseTypeId = values[2] as number;
      ...
      case 20: {
        ...
        const sample = {
          ...
          hr: values[3],
          cadence: values[4],
          targetPower: null as number | null,
        };
        const tgtKey = `${DEV_DATA_INDEX}:0`;
        if (devValues[tgtKey] != null) {
          const raw = devValues[tgtKey];
          sample.targetPower = Array.isArray(raw) ? null : (raw as number);
        }
        recordSamples.push(sample);
        break;
      }
      case 27: {
        const msgIdx = (values[254] as number) || 0;
        const durValue = (values[2] as number) || 0;
        const durSec =
        ...
        const startPct =
          devValues[`${DEV_DATA_INDEX}:1`] != null
            ? (devValues[`${DEV_DATA_INDEX}:1`] as number) / 100
            : null;
        const endPct =
          devValues[`${DEV_DATA_INDEX}:2`] != null
            ? (devValues[`${DEV_DATA_INDEX}:2`] as number) / 100
            : null;
        workoutSteps[msgIdx] = {
          durationSec: durSec,
          durationType,
      ...
      case 26: {
        workoutMeta.name = (values[0] as string) || workoutMeta.name;
        def.devFields.forEach((f) => {
          const info = devFieldInfo.get(`${f.devIndex}:${f.num}`);
          if (!info) return;
      ...
      case 21: {
        ...
        if (ts != null && type != null) {
          timerEvents.push({ ts: ts as number, type: type as number });
        }
        break;
      }
      default:
  ...
  if (sessionValues[2] != null) {
    startTime = fitTimestampToDate(sessionValues[2] as number);
  } else if (recordSamples.length) {
    startTime = fitTimestampToDate((recordSamples[0] as { t: number }).t);
  }

  const samples: FitSample[] = recordSamples.map((s) => {
    const relT = startTime ? s.t - dateToFitTimestamp(startTime) : 0;
    return {
      t: relT,
      power: s.power as number | null,
      hr: s.hr as number | null,
      cadence: s.cadence as number | null,
      targetPower: s.targetPower,
    };
  });

  let canonicalWorkout: CanonicalWorkout | null = null;
  ...
  if (!canonicalWorkout) {
    const ftp = (sessionValues[57] as number) || 0;
    const rawSegments: unknown[][] = [];
    const stepList = workoutSteps.filter(Boolean) as NonNullable<
      (typeof workoutSteps)[number]
    >[];
    ...
    stepList.forEach((s) => {
      ...
      const startPct =
        s.startPct != null
          ? s.startPct
          : (((s.customLow as number) || 0) / ftpSafe) * 100;
      const endPct =
        s.endPct != null
          ? s.endPct
          : ((s.customHigh != null
              ? (s.customHigh as number)
              : (s.customLow as number) || 0) /
              ftpSafe) *
            100;
    ...
    canonicalWorkout = {
      source: (workoutDevFieldValues['vd_source'] as string) || 'Unknown',
      sourceURL: (workoutDevFieldValues['vd_src_url'] as string) || '',
      workoutTitle: workoutMeta.name || 'Workout',
      rawSegments: rawSegments as CanonicalWorkout['rawSegments'],
      description: (workoutDevFieldValues['vd_desc'] as string) || '',
    };
  }

  const meta = {
    ftp: (sessionValues[57] as number) || null,
    startedAt: startTime,
    endedAt:
      sessionValues[253] != null
        ? fitTimestampToDate(sessionValues[253] as number)
        : recordSamples.length
          ? fitTimestampToDate(
              (recordSamples[recordSamples.length - 1] as { t: number }).t,
            )
          : null,
    totalWorkJ: (sessionValues[41] as number) || null,
    totalElapsedSec:
      sessionValues[7] != null ? (sessionValues[7] as number) / 1000 : null,
    totalTimerSec:
      sessionValues[8] != null ? (sessionValues[8] as number) / 1000 : null,
