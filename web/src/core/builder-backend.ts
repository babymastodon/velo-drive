...
export interface BlockAttrs {
  ...
  onPowerRel?: number;
  offPowerRel?: number;
  onCadenceRpm?: number | null;
  offCadenceRpm?: number | null;
}
...
export interface TextEventAnchor {
  blockId: number;
  segIndex: number;
  offsetRel: number;
}

export interface AnchoredTextEvent {
  offsetSec: number;
  durationSec: number;
  text: string;
  anchor: TextEventAnchor | null;
}
...
interface HistorySnapshot {
  meta: BuilderMeta;
  blocks: Block[];
  textEvents: AnchoredTextEvent[];
  selectedBlockIndex: number | null;
  insertAfterOverrideIndex: number | null;
}
...
export function createBuilderBackend() {
  ...
  const state = {
    ...
    currentRawSegments: [] as RawSegment[],
    currentErrors: [] as { field?: string; message: string }[],
    currentBlocks: [] as Block[],
    currentMetrics: null as SegmentMetrics | null,
    currentZone: null as string | null,
    persistedState: null as unknown,
    textEvents: [] as AnchoredTextEvent[],
    selectedBlockIndex: null as number | null,
    selectedBlockIndices: [] as number[],
    selectionAnchorIndex: null as number | null,
    selectionAnchorCursorIndex: null as number | null,
    insertAfterOverrideIndex: null as number | null,
    undoStack: [] as HistorySnapshot[],
    redoStack: [] as HistorySnapshot[],
    historyGroupHasUndo: false,
    historyPendingSnapshot: null as HistorySnapshot | null,
    isHistoryRestoring: false,
  };

  function getMeta(): BuilderMeta {
    return { ...state.meta };
  }

  function setMeta(nextMeta: Partial<BuilderMeta>): void {
    state.meta = { ...state.meta, ...(nextMeta || {}) };
  }

  function getCanonicalState(): CanonicalWorkout {
    return {
  ...
  function setPersistedState(snapshot: unknown): void {
    state.persistedState = snapshot || null;
  }

  function getPersistedState(): unknown {
    return state.persistedState;
  }

  function getCurrentBlocks(): Block[] {
    return state.currentBlocks;
  }

  function getCurrentRawSegments(): RawSegment[] {
    return state.currentRawSegments;
  }

  function getCurrentMetrics(): SegmentMetrics | null {
    return state.currentMetrics;
  }

  function getCurrentZone(): string | null {
    return state.currentZone;
  }

  function getCurrentErrors(): { field?: string; message: string }[] {
    return state.currentErrors;
  }

  function getTextEvents(): TextEvent[] {
    return stripTextEventAnchors(resolveTextEvents(state.textEvents));
  }

  function setTextEvents(events: Partial<AnchoredTextEvent>[]): void {
    state.textEvents = normalizeTextEvents(events, state.currentBlocks);
  }

  function addTextEvent(event: Partial<AnchoredTextEvent>): number {
  ...
  function updateTextEvent(
    index: number,
    updates: Partial<AnchoredTextEvent>,
  ): void {
    ...
    const merged: Partial<AnchoredTextEvent> = {
      ...current,
      ...(updates || {}),
    };
  ...
  function deleteTextEvent(index: number): void {
    if (!Number.isFinite(index) || index < 0) return;
    if (!Array.isArray(state.textEvents) || !state.textEvents[index]) return;
    recordHistorySnapshot();
  ...
  function getSelectedBlockIndex(): number | null {
    return state.selectedBlockIndex;
  }

  function getSelectedBlockIndices(): number[] {
    return state.selectedBlockIndices.slice();
  }

  function getInsertAfterOverrideIndex(): number | null {
    return state.insertAfterOverrideIndex;
  }

  function setInsertAfterOverrideIndex(idx: number | null): void {
  ...
  function setDefaultBlocks(): void {
  ...
  function recomputeDerived(ftp: number): void {
    state.currentRawSegments = buildRawSegmentsFromBlocks(state.currentBlocks);
    state.currentErrors = [];
    if (
  ...
  function setInsertAfterIndex(idx: number | null): void {
    const next =
      idx == null ||
      !Number.isFinite(idx) ||
      idx < 0 ||
  ...
  function setSelectionRange(
    targetIndex: number,
    options: { preserveInsert?: boolean } = {},
  ): void {
  ...
  function clampCursorIndex(val: number): number {
  ...
  function shiftMoveSelection(direction: number): void {
    if (!state.currentBlocks || !state.currentBlocks.length) return;
    const current =
      state.insertAfterOverrideIndex != null
    ...
    if (state.selectedBlockIndices.length) {
      ...
      const anchorCursor = clampCursorIndex(direction > 0 ? anchor - 1 : anchor);
  ...
  function getSelectedIndicesSorted(): number[] {
    return state.selectedBlockIndices.slice().sort((a, b) => a - b);
  }

  function startHistoryGroup(): void {
  ...
  function applyHistorySnapshot(snapshot: HistorySnapshot | null): void {
    if (!snapshot) return;
  ...
  function cloneTextEvents(
    events: AnchoredTextEvent[] | undefined,
  ): AnchoredTextEvent[] {
  ...
  function getHistoryStatus(): { canUndo: boolean; canRedo: boolean } {
    return {
  ...
  function buildBlockTimings(blocks: Block[]): {
    timings: BlockTiming[];
    totalSec: number;
  } {
  ...
  function buildSegmentTimings(blocks: Block[]): {
  ...
  function buildRawSegmentsFromBlocks(blocks: Block[]): RawSegment[] {
  ...
  function segmentsToBlocks(segments: RawSegment[]): Block[] {
    ...
    for (const seg of segments as unknown as unknown[][]) {
      ...
      const toRel = (v: number) => (v <= 5 ? v : v / 100);
  ...
  function getRawCadence(seg: unknown[]): number | null {
    ...
    if (Number.isFinite(seg[4] as number)) return normalizeCadence(seg[4] as number);
    if (typeof seg[3] === 'number' && Number.isFinite(seg[3])) {
      return normalizeCadence(seg[3] as number);
    }
    return null;
  }
  ...
  function blocksSimilarSteady(
    ...
    const pDiff = Math.abs((a.powerRel || 0) - (b.powerRel || 0));
  ...
  function snapPowerRel(rel: number): number {
    const snapped = Math.round(rel * 20) / 20;
    return Math.max(0.05, snapped);
  }

  function clampRel(val: number | undefined): number {
    return Math.max(0.05, Number.isFinite(val as number) ? (val as number) : 0);
  }

  function snapDurationSec(sec: number): number {
    const step = getDurationStep(sec);
    const snapped = Math.round((Number(sec) || 0) / step) * step;
    return Math.max(1, snapped);
  }

  function getDurationStep(sec: number): number {
  ...
  function commitBlocks(
    updatedBlocks: Block[],
    options: { selectIndex?: number | null } = {},
  ): void {
    ...
    const updatedBlocks = state.currentBlocks.map((block, idx) => {
      ...
      if (nextBlock.kind === 'warmup' || nextBlock.kind === 'cooldown') {
        ...
        if (Number.isFinite(low as number) && Number.isFinite(high as number)) {
          if (nextBlock.kind === 'warmup' && (high as number) < (low as number)) {
  ...
  function deleteSelectedBlock(): void {
    ...
    const indices = state.selectedBlockIndices.slice().sort((a, b) => a - b);
  ...
  function moveSelectedBlock(direction: number): void {
    if (
      state.selectedBlockIndex == null ||
      !state.currentBlocks ||
      !state.currentBlocks[state.selectedBlockIndex]
  ...
  function getBlockDurationSec(block: Block | null): number {
    if (!block) return 0;
  ...
  function getBlockSteadyPower(block: Block | null): number {
    ...
    return Number.isFinite(power as number) ? (power as number) : 0;
  }

  function getBlockCadence(block: Block | null): number | null {
    if (!block) return null;
    const cadence = block.attrs?.cadenceRpm;
    return normalizeCadence(cadence);
  }

  function getRampLow(block: Block | null): number {
    if (!block) return 0;
    const low = block.attrs?.powerLowRel;
    return Number.isFinite(low as number) ? (low as number) : 0;
  }

  function getRampHigh(block: Block | null): number {
    if (!block) return 0;
    const high = block.attrs?.powerHighRel;
    return Number.isFinite(high as number) ? (high as number) : 0;
  }

  function getIntervalParts(block: Block | null): IntervalParts {
  ...
  function createBlock(kind: BlockKind, attrs: BlockAttrs): Block {
    const base: Block = {
      id: allocateBlockId(),
      kind,
      attrs: { ...(attrs || {}) },
  ...
  function buildSegmentsForBlock(block: Block): BlockSegment[] {
    if (!block) return [];
  ...
  function clampDuration(val: number | undefined): number {
    const n = Number(val);
    return Math.max(1, Math.round(Number.isFinite(n) ? n : 0));
  }

  function clampRepeat(val: number | undefined): number {
    const n = Number(val);
    return Math.max(1, Math.round(Number.isFinite(n) ? n : 1));
  }

  function clampPowerPercent(val: number | undefined): number {
    const n = Number(val);
    return Math.max(0, Math.round(Number.isFinite(n) ? n : 0));
  }

  function normalizeCadence(val: number | null | undefined): number | null {
    if (val == null || (val as unknown) === '') return null;
    const n = Number(val);
  ...
  function normalizeTextEvent(
    evt: Partial<AnchoredTextEvent> | null | undefined,
    blocks: Block[],
  ): AnchoredTextEvent {
  ...
  function normalizeTextEvents(
    events: Partial<AnchoredTextEvent>[] | undefined,
    blocks: Block[],
  ): AnchoredTextEvent[] {
    return Array.isArray(events)
  ...
  function normalizeTextEventAnchor(
    anchor: TextEventAnchor | null | undefined,
  ): TextEventAnchor | null {
  ...
  function resolveTextEvents(events: AnchoredTextEvent[]): AnchoredTextEvent[] {
    return normalizeTextEvents(events, state.currentBlocks);
  }

  function stripTextEventAnchors(events: AnchoredTextEvent[]): TextEvent[] {
    return Array.isArray(events)
      ? events.map((evt) => ({
          offsetSec: Number(evt?.offsetSec) || 0,
          durationSec: Number(evt?.durationSec) || 0,
  ...
  function resolveOffsetFromAnchor(
    anchor: TextEventAnchor,
    blocks: Block[],
  ): { offsetSec: number; anchor: TextEventAnchor } | null {
  ...
  function anchorTextEventToBlocks(
    offsetSec: number,
    blocks: Block[],
  ): { offsetSec: number; anchor: TextEventAnchor } | null {
  ...
  function findSegmentAtTime(
    ...
    for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += 1) {
      ...
      for (let segIndex = 0; segIndex < segs.length; segIndex += 1) {
        ...
        if (
          clamped < end ||
          (blockIndex === blocks.length - 1 && segIndex === segs.length - 1)
        ) {
          return {
            blockId: block?.id,
            segIndex,
  ...
  function findBlockById(
    blocks: Block[],
    blockId: number,
  ): { blockIndex: number; block: Block; blockStart: number } | null {
  ...
  function allocateBlockId(): number {
  ...
  function syncTextEventsToBlocks(
    prevBlocks: Block[],
    nextBlocks: Block[],
    events: AnchoredTextEvent[],
  ): AnchoredTextEvent[] {
    ...
    return events.map((evt) => {
      const baseOffset =
        evt?.anchor && prevList.length
          ? resolveOffsetFromAnchor(evt.anchor, prevList)?.offsetSec ??
            evt.offsetSec
          : evt.offsetSec;
      if (evt?.anchor) {
  ...
  function insertTextEventsAtInsertionPoint(
    textEvents: Partial<AnchoredTextEvent>[],
  ): void {
    ...
    const inserted: AnchoredTextEvent[] = textEvents.map((evt) => ({
  ...
  function buildTextEventsForInsertion(
    textEvents: Partial<AnchoredTextEvent>[],
    insertIndex: number,
    ...
    return textEvents
      ...
      .map((evt): AnchoredTextEvent | null => {
      ...
      .filter((x): x is AnchoredTextEvent => x != null);
  }

  function getBlocksDurationUpTo(blocks: Block[], endIndex: number): number {
  ...
  function getTextEventsForSelection(): TextEvent[] {
  ...
  function getInsertAfterIndex(): number | null {
    if (state.insertAfterOverrideIndex != null) {
      return state.insertAfterOverrideIndex;
    }
  ...
  function insertBlockAtInsertionPoint(
    spec: BlockSpec,
    options: { selectOnInsert?: boolean } = {},
  ): void {
    ...
    const insertIndex =
      insertAfterIndex == null ? 0 : Math.max(0, insertAfterIndex + 1);
  ...
  function insertBlocksAtInsertionPoint(
    blocks: Block[],
    options: {
  ...
  function buildBlockFromSpec(spec: BlockSpec | null): Block | null {
    if (!spec) return null;
  ...
  function adjustAdjacentRampsForSteady(
    blocks: Block[],
    insertIndex: number,
    steadyBlock: Block,
  ): void {
    ...
    if (prev && (prev.kind === 'warmup' || prev.kind === 'cooldown')) {
      ...
      const canUpdate =
        prev.kind === 'warmup' ? steadyPower > start : steadyPower < start;
      if (canUpdate) {
        prev.attrs = {
          ...(prev.attrs || {}),
    ...
    if (next && (next.kind === 'warmup' || next.kind === 'cooldown')) {
      ...
      const canUpdate =
        next.kind === 'warmup' ? steadyPower < end : steadyPower > end;
      if (canUpdate) {
        next.attrs = {
          ...(next.attrs || {}),
  ...
  function buildContextualRampBlock(
    kind: BlockKind,
    fallbackBlock: Block,
  ): Block | null {
    ...
    let low: number | null = prevPower ? prevPower.end : null;
    let high: number | null = nextPower ? nextPower.start : null;
  ...
  function getBlockStartEnd(block: Block | null): StartEnd | null {
    if (!block) return null;
    const segs = Array.isArray(block.segments) ? block.segments : [];
    if (segs.length) {
      const first = segs[0]!;
    ...
    if (block.kind === 'steady') {
      ...
      if (Number.isFinite(power as number)) {
        return { start: power as number, end: power as number };
      }
      ...
      if (Number.isFinite(low as number) && Number.isFinite(high as number)) {
        return { start: low as number, end: high as number };
      }
    } else if (block.kind === 'intervals') {
      const on = attrs.onPowerRel;
      const off = attrs.offPowerRel;
      if (Number.isFinite(on as number) && Number.isFinite(off as number)) {
        return { start: on as number, end: off as number };
      }
    }
  ...
  function syncAdjacentRampLinks(
    blocks: Block[],
    blockIndex: number,
    oldStartEnd: StartEnd,
    newStartEnd: StartEnd,
  ): void {
    const EPS = 1e-6;
  ...
  function applyPowerUpdatesAroundCursor(
    prevIndex: number,
    nextIndex: number,
    delta: number,
  ): void {
    const updates: { idx: number; attrs: Partial<BlockAttrs> }[] = [];

    const collectPowerUpdate = (idx: number, position: 'start' | 'end') => {
      ...
      if (b.kind === 'steady') {
        ...
        updates.push({
          idx,
          attrs: {
            [isStart ? 'powerLowRel' : 'powerHighRel']: clampRel(current + delta),
          },
        });
    ...
    const oldStartEnds = new Map<number, StartEnd | null>();
...
export type BuilderBackend = ReturnType<typeof createBuilderBackend>;
